    ...
    <Link href={`/product/${product.slug}`} className="group block">
      <div className={`relative bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-shadow h-full flex flex-col ${isOutOfStock ? 'opacity-75' : ''}`}>
        ...
        <div className="p-3 sm:p-4 flex flex-col flex-grow">
          ...
          <h3 className="text-sm sm:text-lg font-heading font-semibold text-brand-dark mb-1 line-clamp-2 sm:line-clamp-1 leading-tight">{product.name}</h3>
          
          <div className="mt-auto pt-4 flex items-end justify-between">
            <div>
              <span className="text-lg sm:text-xl font-bold text-gray-900">₹{product.base_price}</span>
              <span className="text-[10px] sm:text-sm text-gray-500 ml-1">
